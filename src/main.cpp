...
uint256 hashGenesisBlock("0x0000068235884052cf20e39d2ee39af999af02f3e66a14c6af1e4252f796bf42");

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BITTX: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BITTX Signed Message:\n";
...
{
    // BITTX: IsDust() detection disabled, allows any valid dust to be relayed.
...
// Launch specs
static const int64 nTargetTimespan = 3 * 60; // BITTX: each block
static const int64 nTargetSpacing = 1 * 60; // BITTX: 300 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BITTX: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, BITTX - DarkGravity, written by Evan Duffield - evan@BITTX.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BITTX: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // BITTX: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
      ...
      //vMerkleTree: 3f66108684400d84e2ffb528360d534fa19ecaf47c7c4c2ca1d1352ec4a351ad
        ...
        // Genesis block
        const char* pszTimestamp = "BITTXX";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x3257a28530a3d37e25935c5c0cc6ed49838b27336805a6e109a1bc69cfeb8ff8"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa6, 0xd2, 0xc3, 0xf6 }; // BITTX: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BITTXMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BITTXMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BITTXMiner(CWallet *pwallet)
{
    printf("BITTXMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("BITTX-miner");
    ...
    try { while (true) {
        ...
        printf("Running BITTXMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BITTXMiner, pwallet));
}
